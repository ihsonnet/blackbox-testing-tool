    ...
    <v-row>
      ...
      >
        ...
        <v-form class="ma-3" ref="form" v-model="valid" lazy-validation>
          ...
          <v-row
            class="text-field"
            v-for="(input, idx) in requirements"
            :key="idx"
          >
            <v-col>
              <v-select
            ...
            <v-col cols="2">
              <v-btn
                ...
                @click="deleteReq(input, requirements)"
                justify="center"
                rounded
                elevation="0"
                class="mx-auto"
              ...
              <v-checkbox
                ...
                :rules="[
                  issue == true ||
                    ecp == true ||
                    vba == true ||
                    'You must Select at least one calculate method.',
                ]"
                label="Testing Issue"
              ></v-checkbox>
            </v-col>
        ...
        <v-row v-if="vba">
          <v-col>
            ...
            <v-card v-if="!isRange"
              ...
              "
            >
            <v-card-text class="error lighten-3">No Range Found !</v-card-text>
            </v-card>
            <v-card v-if="isRange"
              class="mx-auto pa-5"
              elevation="0"
            ...
            >
              ...
              >
                <v-col>
                  <div style="text-align: right">
                    {{ minBoundary - 1
                      ...
                      ><b>{{ minBoundary }}</b></span
                    >
                  </div>
                </v-col>
                <v-col>
                  <div style="margin-left: 10px; text-align: left">
                    {{ parseInt(minBoundary) + 1
                    }}<span style="text-align: right; margin-left: 190px">{{
                      maxBoundary - 1
                    }}</span>
                  </div>
                </v-col>
                <v-col>
                  <div style="text-align: left">
                    <span style="margin-right: 30px; color: red"
                      ><b>{{ maxBoundary }}</b></span
                    >{{ parseInt(maxBoundary) + 1 }}
                  </div>
                </v-col>
              </v-row>
              <v-row class="mx-auto" style="max-width: 800px">
                <v-col cols="6" style="text-align: right; padding-right: 40px">
                  <div>
                    Boundary-1 <span style="margin-left: 50px">Boundary+1</span>
                  </div>
                </v-col>
                <v-col cols="6" style="text-align: left; padding-left: 60px">
                  <div>
                    <span style="margin-right: 50px">Boundary-1</span>Boundary+1
                  </div>
                </v-col>
              </v-row>
              ...
              <v-row>
                <v-col>
                  <br />
                  <b>Test Cases are : </b> {{ parseInt(minBoundary) - 1 }} ,
                  {{ parseInt(minBoundary) }} ,
                  {{ parseInt(minBoundary) + 1 }} ,
                  {{ parseInt(maxBoundary) - 1 }} ,
                  {{ parseInt(maxBoundary) }} , {{ parseInt(maxBoundary) + 1 }}
                  <br />
                  <br />
                  <table>
                    <tr>
